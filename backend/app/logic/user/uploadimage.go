...
func LogicUserUploadImage(c *gin.Context,req *types.UploadImageReq)(resp *types.UploadImageResp,code int,err error){
	u,ex:=c.Get("id")
	if !ex{
	...
	if user!=nil{
		//用户存在
		//生成存储路径
		absPath := fmt.Sprintf("%s/%d%s",global.Cfg.Image.AvatarsPath, user.ID,path.Ext(req.File.Filename))
		//删除旧头像（如果存在）
